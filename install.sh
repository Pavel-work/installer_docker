...
choices=$(dialog --stdout \
                 ...
                 --ok-label "Установить" \
                 --cancel-label "Отмена" \
                 --checklist "Выберите какие сервисы хотите установить:" 24 90 8 \
...
if [ $? -ne 0 ]; then
    dialog --title "Отмена" --msgbox "Установка отменена." 10 80
    clear
    exit 0
fi

if [ -z "$choices" ]; then
    dialog --title "Ошибка" --msgbox "Вы не выбрали ни одного сервиса!\nУстановка прервана." 12 80
    clear
    exit 1
fi
...
# ==========================================
dialog --title "Готово" --msgbox "Установка успешно завершена!" 10 80
