#!/bin/bash
# Универсальный установщик сервисов v2.1
...
save_selected_services() {
  mkdir -p "$STATE_DIR"
  # Фикс: корректный вывод массива с переносами строк
  printf "%s\n" "${SELECTED_ARRAY[@]}" > "$SELECTED_FILE"
}

load_selected_services() {
  SELECTED_ARRAY=()
  # Фикс: mapfile не добавляет пустую строку в конец
  [[ -f "$SELECTED_FILE" ]] && mapfile -t SELECTED_ARRAY < <(grep -v '^$' "$SELECTED_FILE" 2>/dev/null || true)
}
...
load_params() {
  if [[ -f "$PARAMS_FILE" ]]; then source "$PARAMS_FILE"; fi
...
# === Подсказка для ввода (Исправлена проблема с вставкой) ===
INPUT_HINT="
[Вставка: Shift+Insert или ПКМ мыши]
[Копирование: Выделите текст мышью или Ctrl+Shift+C]
...
show_service_menu() {
  ...
  dialog --clear --title "Шаг 1: Выбор сервисов" \
    ...
    "${args[@]}" 2> "$TEMP_FILE"
  local res=$?
  if [ $res -eq 3 ] || [ $res -eq 1 ]; then return 1; fi
...
step_postgres() {
  dialog --clear --title "Шаг 2: PostgreSQL" \
    ...
    --inputbox "Введите пароль admin (пусто = сгенерировать автоматически)$INPUT_HINT" 14 70 "$PGPASSWORD" 2> "$TEMP_FILE"
  local res=$?
  [ $res -eq 3 ] && return 1; [ $res -eq 1 ] && return 1
...
step_jwt() {
  dialog --clear --title "Шаг 3: JWT Secret" \
    --extra-button --extra-label "Назад" --ok-label "Далее" \
    --inputbox "JWT Secret (нужен для Supabase и др. Пусто = сгенерировать)$INPUT_HINT" 14 70 "$JWT_SECRET" 2> "$TEMP_FILE"
  local res=$?
  [ $res -eq 3 ] && return 1; [ $res -eq 1 ] && return 1
...
step_ports() {
  while true; do
    ...
    [[ " ${SELECTED_ARRAY[@]} " =~ "n8n" ]] && { form_args+=("Порт n8n:" $row 2 "$N8N_PORT" $row 20 15 0); ((row+=2)); }
    [[ " ${SELECTED_ARRAY[@]} " =~ "apache" ]] && { form_args+=("Порт Apache:" $row 2 "$APACHE_HTTP_PORT" $row 20 15 0); ((row+=2)); }
    [[ " ${SELECTED_ARRAY[@]} " =~ "qdrant" ]] && { form_args+=("Порт Qdrant:" $row 2 "$QDRANT_PORT" $row 20 15 0); ((row+=2)); }
    [[ " ${SELECTED_ARRAY[@]} " =~ "ollama" ]] && { form_args+=("Порт Ollama:" $row 2 "$OLLAMA_PORT" $row 20 15 0); ((row+=2)); }
    [ ${#form_args[@]} -eq 0 ] && return 0

    local height=$((row + 5)); [ $height -gt 20 ] && height=20
    dialog --clear --title "Шаг 6: Порты сервисов" \
      --extra-button --extra-label "Назад" --ok-label "Далее" \
      --form "Укажите порты (Tab для перехода)$INPUT_HINT" $height 70 0 "${form_args[@]}" 2> "$TEMP_FILE"
    local res=$?
    [ $res -eq 3 ] && return 1; [ $res -eq 1 ] && return 1

    local ports; mapfile -t ports < "$TEMP_FILE"
    local idx=0 ports_ok=true
    [[ " ${SELECTED_ARRAY[@]} " =~ "n8n" ]] && { N8N_PORT="${ports[$idx]}"; check_port "$N8N_PORT" || ports_ok=false; ((idx++)); }
    [[ " ${SELECTED_ARRAY[@]} " =~ "apache" ]] && { APACHE_HTTP_PORT="${ports[$idx]}"; check_port "$APACHE_HTTP_PORT" || ports_ok=false; ((idx++)); }
    [[ " ${SELECTED_ARRAY[@]} " =~ "qdrant" ]] && { QDRANT_PORT="${ports[$idx]}"; check_port "$QDRANT_PORT" || ports_ok=false; ((idx++)); }
    [[ " ${SELECTED_ARRAY[@]} " =~ "ollama" ]] && { OLLAMA_PORT="${ports[$idx]}"; check_port "$OLLAMA_PORT" || ports_ok=false; ((idx++)); }
    [ "$ports_ok" = true ] && break
  done
...
# === 4. Supabase (ИСПРАВЛЕНО: sed, networks, генерация ключей) ===
setup_supabase() {
  ...
  if [ ! -d "supabase-docker" ]; then
    ...
    cd supabase && git sparse-checkout set docker utils > /dev/null 2>&1 && cd ..
  ...
  # Фикс: используем | вместо / в sed, чтобы не ломать base64/hex ключи
  ...
  # Фикс: убрано дублирование networks:, достаточно docker network connect ниже
...
# === 5. Генерация docker-compose.yml (ИСПРАВЛЕНО: n8n env, webhook) ===
...
EOF
  ...
  if [[ " ${SELECTED_ARRAY[@]} " =~ "n8n" ]]; then
    # Фикс: корректный YAML и динамический WEBHOOK_URL
    cat >> docker-compose.yml <<EOF
  n8n:
...
start_containers() {
  ...
  if [[ " ${SELECTED_ARRAY[@]} " =~ "n8n" ]] && [ "${N8N_DB_POSTGRES:-0}" -eq 1 ]; then
    ...
    while ! docker exec postgres pg_isready -U admin &>/dev/null; do
      sleep 2; timeout=$((timeout - 2)); [ $timeout -le 0 ] && break
    done
    docker exec postgres psql -U admin -c "CREATE DATABASE n8n;" 2>/dev/null || true
  fi
}

# === 7. SSL для NPM (Автоматическая настройка через API) ===
configure_npm_ssl() {
  [[ ! " ${SELECTED_ARRAY[@]} " =~ "nginx_proxy" ]] || [ -z "$DOMAIN" ] && return 0
  echo "### Ожидание запуска Nginx Proxy Manager..."
  sleep 10
  ...
  if [ "$TOKEN" == "null" ] || [ -z "$TOKEN" ]; then
    echo -e "${YELLOW}⚠ Не удалось авторизоваться в NPM API. Настройте SSL вручную в панели.${NC}"
    return 1
  fi

  # Определяем целевой сервис для проксирования
  local forward_host="apache"
  local forward_port=80
  [[ " ${SELECTED_ARRAY[@]} " =~ "n8n" ]] && { forward_host="n8n"; forward_port=5678; }

  echo "### Создание прокси-хоста и запрос SSL для $DOMAIN..."
  local res
  ...
  if echo "$res" | jq -e '.id > 0' &>/dev/null; then
    ...
    echo -e "${YELLOW}⚠ Ошибка API NPM: $(echo "$res" | jq -r '.error.message // "Неизвестная ошибка"')${NC}"
    echo "   Настройте SSL вручную в панели NPM (http://$(hostname -I | awk '{print $1}'):81)"
  fi
}

# === 8. Полное удаление (Wipe) ===
full_cleanup() {
  dialog --yesno "⚠️ ВНИМАНИЕ: Это полностью удалит все контейнеры, сети, тома и конфигурацию.\nДанные будут УДАЛЕНЫ БЕЗ ВОЗВРАТА.\nПродолжить?" 10 60 || return 0
  
  echo -e "${YELLOW}🧹 Остановка контейнеров...${NC}"
  docker compose -p supabase down -v 2>/dev/null || true
  cd "$SETUP_DIR" 2>/dev/null && docker compose down -v 2>/dev/null || true
  
  echo -e "${YELLOW}🧹 Удаление образов и кэша...${NC}"
  docker system prune -af --volumes 2>/dev/null || true
  
  echo -e "${YELLOW}🧹 Очистка директорий...${NC}"
  rm -rf "$SETUP_DIR" "$STATE_DIR"
  
  dialog --msgbox "✅ Сервер полностью очищен.\nМожете перезапустить установщик." 10 50
  exit 0
}
...
run_installation_process() {
  {
    echo "5"; echo "### Проверка системы и установка Docker..."
    install_docker
    echo "15"; echo "### Настройка внутренней сети..."
    setup_network
    echo "30"; echo "### Развертывание Supabase..."
    setup_supabase
    echo "60"; echo "### Генерация конфигурации..."
    generate_compose_file
    echo "80"; echo "### Запуск контейнеров..."
    start_containers
    echo "95"; echo "### Настройка SSL..."
    configure_npm_ssl
...
main() {
  ...
  if ! command -v dialog &> /dev/null; then apt-get update > /dev/null && apt-get install -y dialog jq > /dev/null; fi
  ...
  case "$current_state" in
    "start")
      ...
      ;;
    "completed")
      dialog --menu "Установка завершена. Действие:" 14 60 4 \
