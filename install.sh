...
# Подсказка для ввода
INPUT_HINT="
\n\n[Вставка: Shift+Insert или Ctrl+Shift+V]
\n[Копирование: Выделите мышью или Ctrl+Shift+C]
\n[Навигация: Tab для перехода к кнопкам]"

# === 2. Ввод параметров (с навигацией назад) ===
step_postgres() {
    local hint="Введите пароль admin (пусто = сгенерировать автоматически)$INPUT_HINT"
